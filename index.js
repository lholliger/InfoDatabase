...
async function generateAuthorNameString(articleName) {
  ...
  return `${firstNames[parseInt(`${hashHex[0]}${hashHex[1]}`, 16)]} ${
    lastNames[parseInt(`${hashHex[2]}${hashHex[3]}`, 16)]
  }`;
}
...
async function handleRequest(request) {
  ...
  if (uri.startsWith("/article/")) {
    ...
    let meta = await fetch(
      `https://en.wikipedia.org/w/rest.php/v1/page/${articleName}/bare`
    );
    meta = await meta.json();

    let content = await fetch(
      `https://en.wikipedia.org/w/rest.php/v1/page/${articleName}/html`
    );
    ...
    content = content.replace(
      "%%ARTICLEINFO%%",
      `<h1>${meta.title}</h1><i>Written by ${author}. Last updated ${new Date(
    ...
    content = content.replaceAll(
      /Main article.*\: /g,
      "&emsp;&emsp;<b>More on this topic: </b>"
    );
    content = content.replaceAll(
      /Further information\: /g,
      "&emsp;&emsp;<b>More information: </b>"
    );
    content = content.replaceAll(
      /See also\: /g,
      "&emsp;&emsp;<b>Related: </b>"
    );

    // clean up ending
    content = content.split(
      /<h2 id="Notes">Notes<\/h2>|<h2 id="See\_also">See also<\/h2>/
    )[0];

    content += `
 <div class='idb-footer'>
    <hr><i>
    This article falls under the <a href='${meta.license.url}'>${
      meta.license.title
    }</a> license. For attributions and references on this article, please <a href='/references/${articleName}'>click here</a>.<br><br>
    ...
    let content = `${head.replace("%%ARTICLETITLE%%", "Home")} ${body.replace(
      "%%ARTICLEINFO%%",
      ""
    )}
    ...
    content = `${head.replace(
      "%%ARTICLETITLE%%",
      `References for ${meta.title}`
    )} ${body.replace(
      "%%ARTICLEINFO%%",
      `<h1>Article Information for <a href='/article/${articleName}'>${meta.title}</a></h1>`
    )}`;

    content += `<h2>References</h2>`;
    content += wikiarticle
      .split('<h2 id="References">References</h2>')[1]
      .split("</section>")[0];
    ...
    let content = `${head.replace("%%ARTICLETITLE%%", "About")} ${body.replace(
      "%%ARTICLEINFO%%",
      ""
    )}
